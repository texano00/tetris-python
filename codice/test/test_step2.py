...
class TestStep2(unittest.TestCase): 
    ...
    def test_P1(self):
        ...
        self.assertEqual("user2", data['data']['last_play']['usr'])
        self.assertEqual([1,9], data['data']['play']['user2']['done'])
        ...
        self.assertEqual("user1", data['data']['last_play']['usr'])
        self.assertEqual([5,7], data['data']['play']['user1']['done'])
        ...
        self.assertEqual("user2", data['data']['last_play']['usr'])
        self.assertEqual([1,9,3], data['data']['play']['user2']['done'])
        ...
        self.assertEqual("user1", data['data']['last_play']['usr'])
        self.assertEqual([5,7,4], data['data']['play']['user1']['done'])

        data = addMove('user2',2,data) #USER2 PLAY 2
        self.csocket2.send(message().encode(data))
        ##END USER2

        ##USER2 KNOW THAT USER2 IS THE WINNER
        data = self.csocket2.recv(1024)
        data = message().decode(data)
        self.assertEqual("user2", data['data']['winner'])

        ##USER1 KNOW THAT IS NOT THE WINNER
        data = self.csocket1.recv(1024)
        data = message().decode(data)
        self.assertNotEqual("user1", data['data']['winner'])
    ...
    def test_P2(self):
        ...
        self.assertEqual(5, data['data']['last_play']['move'])
        self.assertEqual("user2", data['data']['last_play']['usr'])
        self.assertEqual([1,5], data['data']['play']['user2']['done'])
        ...
        self.assertEqual("user1", data['data']['last_play']['usr'])
        self.assertEqual([3,6], data['data']['play']['user1']['done'])
...
if __name__=="__main__":
    ...
    if len(part)==0:
        ...
        for t in part:
            ...
            print "Adding test: {}\n".format(testName)
            suite.addTest(TestStep2(testName))
